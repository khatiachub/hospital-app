...
import { Calendar,ChildEvent,EditCalendarEvent,EditEvent } from '../../shared/User.interface';
...
export class CalendarComponent implements OnInit {
  ...
  constructor(private dataService: DataService,public customDateFormatter: CustomDateFormatter) {
  }
  modalData!: {
    action: string;
    event: CalendarEvent;
...
// updateDataInService() {
//   const dataToSend = this.SumUserBooks;
...
// }
    ...
    this.dataService.getEvent().subscribe({
      next: (response) => {
        ...
        response.$values.forEach((response: any) => {
          ...
          response.color =
            response.isBooked && !this.role && id !== response.userId
              ? { ...colors['red'] }
              : { ...colors['green'] };
          response.title =
            !this.editUser && this.BookQuantity > 0
              ? `+ დაჯავშნა დარჩენილია ${this.BookQuantity} ადგილი`
              : (response.isBooked && this.role === 'USER' && this.editUser) ||
                id === response.userId
              ? `ჩემი ჯავშანი  ${this.books}`
              : response.isBooked && this.role === 'DOCTOR' && this.editUser
              ? `დაჯავშნილია  დარჩენილია ${this.booksForDoctor} ადგილი`
              : `დაჯავშნილია  დარჩენილია ${this.BookQuantity} ადგილი`;
        });
        ...
        this.UserBooks = nested.filter(
          (event: Calendar) => event.userId === this.doctorId || event.userId === id
        );
        nested.forEach(
          (event: Calendar) => (this.isUserEntered = event.userId === id)
        );
        this.events.forEach(
          (event:any) => (this.isDoctorEntered = event.doctorId === id)
        );

        this.SumDoctorBooks = this.DoctorBooks.length;
        this.SumUserBooks = this.UserBooks.length;
        this.dataService.updateBook(this.role==='USER'?this.SumUserBooks:this.SumDoctorBooks);
      },
      error: (error) => {
        console.log(error);
      },
  ...
  addEvent(): void {
    ...
    this.inputValue.color = colors['green'].primary;
  ...
  lastName!: string;
  bookDate: Calendar[] = [];
  handleEvent(action: any, event:any): void {
    this.Editable=false;
  ...
  editEvent(): void {
    ...
    const event = this.PlaceEvents.filter((event:Calendar) => event.id === this.Id);
  ...
  deleteEvent(book: ChildEvent) {
    this.data = {
      ...
      isBooked: false,
      description: '',
      calendarModelId: book.calendarModelId,
      id: book.id,
      start: book.start,
      endDate: book.endDate,
    };
    const filtered = this.events.filter(
      (event: CalendarEvent) => event.id === book.calendarModelId
    );
        ...
        next: (response) => {
          ...
          filtered.forEach((event: any) => {
            this.filteredArray = event.Events.$values.filter(
               (childEvent: ChildEvent) => (childEvent.id!==response.id&&childEvent.isBooked)
             );
           });
